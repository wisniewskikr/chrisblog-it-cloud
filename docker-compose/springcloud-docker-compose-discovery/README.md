...
1. Start services with `docker-compose up -d --build`
     * (Optonal) Check if all services are up and running (expected STATUS healthy) with `docker ps -a`
1. Visit service Discovery with `http://localhost:8761`
1. (Optional) Visit service HelloWorld with `http://localhost:8080`
...
##### Goal
The goal of this project is to present how to implement **Discovery Service** in **Java microservice system**. This microservice system consists of Discovery Service and one application in **Java** programming language with usage **Spring Boot Cloud** framework. 
...
1. User via Browser checks all services registered in the system
