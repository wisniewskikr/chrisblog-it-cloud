...
1. Start services with `docker-compose up -d --scale service-helloworld=2 --build`
1. Visit many times service HelloWorld via service Gateway with `http://localhost:8762`
     * *Expected result: Application uuid should be changed because load balancer alternately redirects traffic to two instances of service HelloWorld*
...
1. Service Gateway sends request to Service HelloWorld for content. It checks count of instances in service Discovery. If there are more then one then traffic is redirected alternately
